...
for i in range(n):
 print(students[i])
'''
#Group Classification and initialization 
high_performers_group = {}
...
for every_student in students:
    ...
    student_score = every_student['score']
    student_tuple = (student_name, student_score)
    
    if student_score >= 70:
        high_performers_group[student_id] = student_tuple
    elif student_score >= 50 and student_score < 70:
        average_performers_group[student_id] = student_tuple
    else:
        low_performers_group[student_id] = student_tuple

print("\n**************************************************\n")
print(f"Total number of students in the High Performers Group is: {len(high_performers_group)}")
print(f"Total number of students in the Average Performers Group is: {len(average_performers_group)}")
print(f"Total number of students in the Low Performers Group is: {len(low_performers_group)}")

print("\n*********High Performers Group ********************\n")
for student_id, (student_name, student_score) in high_performers_group.items():
    print(f"  {student_id}: ('{student_name}', {student_score})")

print("\n*********Average Performers Group ******************\n")
for student_id, (student_name, student_score) in average_performers_group.items():
    print(f"  {student_id}: ('{student_name}', {student_score})")

print("\n********Low Performers Group ***********************\n")
for student_id, (student_name, student_score) in low_performers_group.items():
    print(f"  {student_id}: ('{student_name}', {student_score})") 
